// AKV-DATA-020 — kaynaklı tatlı su veri partisi 2 (20 kayıt)
;(() => {
  'use strict';

  const SOURCE_FISHBASE = 'fishbase-freshwater-batch-2-2026';
  const SOURCE_CARE = 'seriouslyfish-freshwater-batch-2-2026';
  const SOURCE_EDITORIAL = 'freshwater-batch-2-editorial-v1';
  const SOURCE_MIGRATION = 'legacy-inhabitant-migration-v1';
  const specs = window.AKV_FRESHWATER_BATCH_2_SPECS || [];

  if (specs.length !== 20) {
    throw new Error(`AKV-DATA-020 parti 2: 20 kayıt bekleniyordu, ${specs.length} bulundu.`);
  }
