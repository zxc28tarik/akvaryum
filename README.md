...
50 kaynaklı tür çifti istisnası `data/curation/compatibility-overrides-v1.json`, sözleşmesi `schemas/compatibility-override-v1.schema.json` dosyasındadır. İstisna motoru genel ikili davranış sonucunun üstüne yazar; pH, sıcaklık, GH, su tipi, tank ve sosyal grup gibi bağımsız güvenlik kurallarını silmez.

Motorun genel sonucu `result.scoreBreakdown` altında dört açıklanabilir puana ayrılır: çevresel uyum 30, davranış/sosyal uyum 30, tank/biyolojik yük 25 ve habitat/bakım uyumu 15. Kritik sorunlar bölüm puanını sıfırlar ve türüne göre genel puana üst sınır uygular; böylece ölüm, avlanma veya kesin tank yetersizliği yüksek toplam puanın arkasında saklanmaz.
...
- `check:engine-pairs`: 50 doğrulanmış tür çifti istisnasını, dokuz kaynak bağlantısını, yönlü/çift yönlü ilişkiyi, dört sonuç durumunu ve çevresel kuralların korunmasını 19 senaryoyla doğrular.
- `check:engine-scores`: 30/30/25/15 alt puanlarını, kritik toplam üst sınırlarını, bölüm açıklamalarını, boş seçim davranışını ve Türkçe/İngilizce çıktıyı 13 senaryoyla doğrular.
...
- Sıralama ad, tank hacmi, yetişkin boyu veya bakım zorluğuna göre yapılabilir.
- Filtreler `q`, `cat`, `care`, `temperament`, `social`, `zone`, `tankMax`, `plantSafe`, `reefSafe` ve `sort` URL alanlarında saklanır.
